export default function pageController() {
  ...
  function pageController($scope, $timeout, $mdSidenav, $log, $location, $routeParams, $http,$route,$cookies, $mdToast) {
    ...
    // console.log(pageCookies,'pageCookies')
    ...
    if(paramValue == 'static'){
        vmPage.phone = pageCookies.MasOassBqqLssIxxE != 'null' ? pageCookies.MasOassBqqLssIxxE : ""
        vmPage.email = pageCookies.email != 'null' ? pageCookies.email : ""
        vmPage.name =  pageCookies.name != 'null' ? pageCookies.name : ""

        console.log(vmPage.email,'vmPage.email')
        console.log(vmPage.name,'vmPage.name')
        console.log(vmPage.phone,'vmPage.phone')
            ...
            $scope.submit = function(){
                ...
                for(var i=0;i<formData.length;i++){
                    ...
                    if(formData[i].value){
                        ...
                        // $mdToast.show( $mdToast.simple().textContent('Details has been submitted'));
                        $mdToast.showSimple('Please fill all the details')
                        return false;
                    }
    ...
    if(paramValue == 'enquiry'){
        ...
        $scope.enquire = function(){
            ...
            for(var i=0;i<formData.length;i++){
                if(formData[i].value){
                    userData[formData[i].name] = formData[i].value
                    // console.log(formData[i].name,'formData[i].name')
                    vmPage.errMsg = ''
                }else{
                    $mdToast.showSimple('Please fill all the details')
                    return false;
