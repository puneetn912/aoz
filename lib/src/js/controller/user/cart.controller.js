	...
	function cartController($scope,$timeout, $q, $log,$location,$routeParams, $http,$cookies,$mdToast) {
        ...
        $http({method:'POST',url:`/getSub/${subId}`}).then(function successCallback(response) { if (response.data){
            ...
            if(!cookies.mealType){
                ...
                let vegCost = 119, nvegCost = 149
                console.log(cookies.countVeg,'cookies.countVeg')
                if(cookies.countVeg == 1){
                    vegCost = 99
                }
                if(cookies.countNveg == 1){
                    nvegCost = 119
                }

                vmCart.vegAmt = ((cookies.countVeg * vegCost)).toFixed(2)  
                vmCart.nvegAmt = ((cookies.countNveg * nvegCost)).toFixed(2)  
        ...
        $scope.paynow = function(payment){
            if(host == 'localhost'){
                ...
                let orderNo = `${vmCart.userPhone.toString().slice(6,10)}${new Date().getTime()}`
