	...
	function calenderController($scope,$timeout, $q,$log,$http,$routeParams,$location,$mdDialog,$mdToast,$cookies) {
        ...
        $http.post(`/getSub/${$routeParams.subId}`).then(function(response){if(response.data){
            ...
            if(response.data.account_status == false){
                ...
                $mdDialog.show(confirm4).then(function() {
                    ...
                    $http.post(`/resumeSub/${response.data._id}`).then(function(response){if(response.data){
                        console.log('resumed')
                    }})
                });
        ...
        $scope.pause = function(){
            ...
            vmCalender.cal.map((x,i)=>{
                ...
                let today = new Date(vmCalender.currDate)
                console.log(calDate, today, 'dates')
                if(calDate <= today){ console.log(calDate,'small bro') }else{
                    console.log(calDate,'adding this')
            ...
            var confirm2 = $mdDialog.confirm()
                      .title(`Attention`)
                      .textContent(`Meal distribution on your calendar will be erased. Are you sure you want to pause subscription from ${vmCalender.currDate.toString().slice(4,15)}?`)
                      .ariaLabel('Lucky day')
                      .ok('Yes')
                      .cancel('No');
                ...
                $mdDialog.show(confirm2).then(function() {
                    console.log('asdf')
                    $http.post('/pauseSub',dataToBeRemoved).then(function(response){if(response.data){
