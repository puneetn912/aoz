export default function calenderController() {
	...
	calenderController.$inject = ['$scope','$timeout', '$q', '$log','$http','$routeParams','$location'];
	function calenderController($scope,$timeout, $q,$log,$http,$routeParams,$location) {
		var vmCalender = this;
        ...
        $scope.calinveg = function(){
            if(vmCalender.nvegRem==0){
                console.log('remaingin 0 bro')
                vmCalender.error = 'Veg Meal remaining is 0'
            }else{
                vmCalender.nvegRem--
                document.getElementById('finalSubmit').style.display = "block"
