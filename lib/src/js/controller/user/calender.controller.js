export default function calenderController() {
	...
	function calenderController($scope,$timeout, $q, $log,$http) {
        ...
        let existingDateVeg = vmCalender.allDatesVeg 
        let existingDateNonveg = vmCalender.allDatesNonveg
        ...
        $scope.finalSave = function(){
            ...
            let finalObj = [{date:vmCalender.startDate.toString().slice(3,15), vegCount:0, nonVegcount:0}]
            vmCalender.allDatesVeg.map((x, index)=>{
                vmCalender.allDatesNonveg.map((y, index)=>{
                    ...
                    if(x.date == y.date){
                        ...
                        if(finalIndexVeg == -1){
                            finalObj.push({date:x.date, vegCount:x.vegCount, nonVegcount:0})
                        }
                        if(finalIndexNonveg == -1){
                            finalObj.push({date:y.date, vegCount:0, nonVegcount:y.nonvegCount})
                        }
