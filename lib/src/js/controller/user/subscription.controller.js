        ...
        $scope.subscribe = function(){
            ...
            if(total!=0 && mealType){
                let data = {
                    vegCount:vegCount, 
                    nvegCount:nvegCount, 
                    subId:$cookies.get('SaweUSeaBaeSaaCaaRxxIssBaaEaaR'), 
                    category:'dinner',
                    mealType:mealType,
                }
                $http({method:'POST',url:`/updateSubs/${$cookies.get('UaweSeaEaeRaaIaaD')}`, data:data}).then(function successCallback(response) { 
                if (response.status == 200){
                    $location.path(`/cart/${$cookies.get('SaweUSeaBaeSaaCaaRxxIssBaaEaaR')}`);
                }});
            }else{
