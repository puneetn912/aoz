	...
	function subscriptionController($scope,$timeout, $q, $log, $filter,$http,$location,$routeParams,$cookies,$mdToast) {
        ...
        $scope.subscribe = function(){
            if(total!=0 && vmSubscription.mealType){
                ...
                if(cookies.sumAmt){
                    totalVegCount = parseInt(cookies.countTotalVeg)
                    totalNvegCount = parseInt(cookies.countTotalNveg)
                    vegRemaining = parseInt(cookies.countVegRem)
                    nvegRemaining = parseInt(cookies.countNvegRem)
                }
                console.log(cookies,'cookiesBro')
                console.log(vegCount,'vegCount', nvegCount,'nvegCount', vmSubscription.sumAmt,'vmSubscription.sumAmt' )
                ...
                // $http.post(`/updateSubs/${$routeParams.subId}`,data).then(function successCallback(response) { 
                    ...
                    $location.path(`/cart`).search({vegCount:null,nonvegCount:null,vegRem:null,nvegRem:null });
                });
            }else{
