export default function signupController() {
	...
	signupController.$inject = ['$scope','$timeout', '$q', '$log','$http','$cookies'];
	function signupController($scope,$timeout, $q, $log, $http, $cookies) {
		var vmSignup = this;
				...
				$http.post(smsurl, JSON.stringify(data)).then(function (response) {
					if(response.data)
					{
						...
						$cookies.put('mobileno', mobile);
			         	window.location.href=`${url[0]}//${window.location.host}/verify`;
