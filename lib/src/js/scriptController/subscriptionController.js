...
exports.createSubs = (req, res, next) => {
    ...
    Subscription.findOneAndUpdate(query, set, {new: true, upsert: true}).populate('user_id apartment_id').exec((err,data)=>{
...
exports.updateStatus = (req,res,next) => {
    ...
    start_date.setHours(5,30)

    let endDate = new Date()
    endDate.setFullYear(endDate.getFullYear() + 1)
    
    let set = {status: true, start_date:start_date, end_date:endDate}

    Subscription.findOneAndUpdate(query, set, {new:true}).populate('user_id').exec((err,data)=>{
        if(err){res.send(err)}else{console.log(data,'subs');res.send(data)}
    })
}
...
exports.getSubNative = (req, res, next) =>{
    //console.log(req.params.id,'param')
    Subscription.findOne({_id:req.params.id}).populate('user_id apartment_id').exec((err,data)=>{
    if(!err){res.send(data)}else{res.send(err)}})
}
...
exports.getSubByUserNative = (req,res,next)=>{
    //console.log(req.params.user,'param')
    let query = {user_id:req.params.user}
    Subscription.findOne(query).populate('user_id apartment_id').exec((err,data)=>{if(err || !data){res.sendStatus(403)}else{
