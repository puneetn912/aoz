    ...
    if(req.body[0].subId){
        ...
        let obj = req.body.map((x,i)=>{
            return {selected_data:x.date, 
                ...
                user_id:x.user_id,
                caterer_veg_id:x.catererVegId,
                caterer_nonveg_id:x.catererNvegId
            }
        })
        console.log(obj,'obj')
...
exports.updateCalOne = (req, res, next) => {
    Calendar.findOne({_id:req.body._id},{$set:{veg_count_on_day:req.body.veg_count_on_day,nonVeg_count_on_day:req.body.nonVeg_count_on_day}}).exec((err, data)=>{
        if(!err){
            res.send(data)
        }else{
            res.send(err)
